...
public class AddSubTask extends AppCompatActivity {
    ...
    int idint,parentid,idinserted;
    String id;
    @Override
    protected void onCreate (Bundle savedInstanceState){
        super.onCreate(savedInstanceState);
        setContentView(R.layout.add_new_subtask);
        ...
        intent = getIntent();
        parentid=intent.getIntExtra("idparent",0);
    ...
    public void init_update(){
        ...
        EditText task_name = (EditText) findViewById(R.id.newsubtasknameid);
        EditText task_description = (EditText) findViewById(R.id.newsubtaskdescriptionid);
        Button task_date = (Button) findViewById(R.id.newsubtaskdateid);
        Cursor task = mydb.getDataSpecific(id);
        TextView toolbar_task_add_title = (TextView) findViewById(R.id.createsubtasknameid);
        toolbar_task_add_title.setText("Update Sub-Task");
        if (task != null) {
            task.moveToFirst();
    ...
    public void goBack(){
        btnForBack=(Button)findViewById(R.id.newsubtaskbackid);
    ...
    public void  onSave(){
        btnForSave=(Button)findViewById(R.id.newsubtasksaveid);
        btnForSave.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                EditText task_name = (EditText) findViewById(R.id.newsubtasknameid);
                EditText task_description = (EditText) findViewById(R.id.newsubtaskdescriptionid);
                btn= (Button) findViewById(R.id.newsubtaskdateid);
                ...
                if (errorStep == 0) {
                    if (isUpdate) {
                        if(mydb.updateContact(id, nameFinal,dateFinal,descriptionFinal)) {
                            Toast.makeText(getApplicationContext(), "Sub-Task Updated.", Toast.LENGTH_SHORT).show();
                        }
                    } else {
                              idinserted=(int) mydb.insertContact(nameFinal, dateFinal,descriptionFinal);
                             mydb.insertParent(idinserted,parentid);
